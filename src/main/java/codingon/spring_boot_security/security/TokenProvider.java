...
public class TokenProvider {
    // JWT 서명에 사용되는 비밀키로 지금은 하드코딩함
    private static final String SECRET_KEY = "sesac-spring-boot-kek3k3k2r25";

    // create(): JWT 생성
    // => 로그인 성공 시에 이 메서드가 호출되어 JWT 토큰을 발급함
    public String create(UserEntity userEntity) {
        // JWT 토큰 만료 시간을 현재시각으로부터 1일뒤 만료되는 날짜로 계산
        Date expiryDate = Date.from(Instant.now().plus(1, ChronoUnit.DAYS));

        // JWT 토큰 생성
        return Jwts.builder()
                // header 에 들어갈 내용 및 서명하기 위한 SECRET KEY
    ...
    // => 클라이언트가 보낸 토큰이 유효한지 검증하고, userId 를 반환
    public String validateAndGetUserId(String token) {
        // parseClaimsJws 메소드가 Base64 로 디코딩 및 파싱
        // - header, payload 를 setSigningKey 로 넘어온 시크릿을 이용해 서명한 후 token 의 서명이랑 비교
        // - 서명이 위조되거나 만료된 토콘이라면 예외 발생
